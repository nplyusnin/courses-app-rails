...
RSpec.describe Teacher::CoursePolicy, type: :policy do
  ...
  shared_examples "createable" do
    ...
    it "grants access to student" do
      expect(subject).to_not permit(student, course)
    end
  end
  ...
  shared_examples "manageable" do
    ...
    it "grants access to teacher owner" do
      expect(subject).to permit(teacher, teacher_course)
    end

    it "denies access to teacher" do
      expect(subject).to_not permit(teacher, course)
    end
